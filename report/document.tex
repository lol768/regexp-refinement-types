...
\begin{figure}[H]
    \begin{MyMdframed}
        ...
        \centering \includegraphics[width=0.5\linewidth]{wso.png}
    \end{MyMdframed}
\end{figure}

LDAP queries are written in a compact manner using operations and parentheses for each clause:

\texttt{(\&(warwickUniId=1510654)(sn=Williams))}

This query retrieves the user with attribute \texttt{warwickUniId} set to value 1510654 and surname (sn) set to value
``Williams''.
...
\begin{figure}[H]
    \begin{MyMdframed}
        ...
        \centering \includegraphics[width=0.5\linewidth]{dobcrack.png}
...
are defined below \citep[p.~102]{sipser2012introduction}:
...
\begin{itemize}
    \item First, all functions under the theory $T$ in the problem are replaced with a Boolean variable.
          DPLL then proceeds as described earlier\footnote{} to find a satisfying valuation for the Boolean variables.
...
\begin{mycodefile}{rrtlex.py:RrtLexer -x}{\label{code:rrt:2}A product function, which multiplies its arguments}{RRT}{integer-refinements.rrt}
    ...
    In this example, we use integer refinement types to restrict the domain of the arguments to the \texttt{Product}
    function.
...
Statements and expressions can inherit from a common abstract base class and reuse their logic.
...
\begin{description}
    ...
    \item[IfStatement] Represents a control flow statement comprising an \textcolor{id7-aubergine}{\texttt{if}} clause and one or more \textcolor{id7-aubergine}{\texttt{else}}/\textcolor{id7-aubergine}{\texttt{else if}} clauses.
...
ANTLR4 generates $LL(*)$ parsers .

\subsection{Integrations and Frontends}

In this section, we present the various tool integrations we have built.
...
Similarly, it is possible that the tooling discussed in this report (Z3, Z3str3 and regular expression refinement
types) could be used to discover potential exploitation opportunities in existing software, perhaps by porting
logic into our language and adding refinement types in order to detect e.g. injection vulnerabilities by running the
type-checker.

\citeauthor{harper2018gray} discusses the ethics of security research in \citet{harper2018gray}.
...
The Full Disclosure mailing list is a common avenue for disclosing vulnerabilities in this manner, with Rose
(the creator of the original list) arguing that other approaches are simply ``security through obscurity''.
