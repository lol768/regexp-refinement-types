...
\begin{algorithmic}
\Function{DPLL}{$\phi$}
	\If{\Call{IsConsistent}{$\phi$}} \Comment Clause contains no $x_1$ and $\neg x_1$
		\State ${\bf return}~\textsc{true}$
	\EndIf
	...
	\For{unit clause $\{l\} \in \phi$} \Comment Unit clauses contain one unassigned literal
		\State $\phi = $ \Call{UnitPropagate}{$l, ~ \phi$}
	\EndFor
...
\todobox{It would be nice to visualise the algorithm's operation on a toy problem here.}
...
\begin{mycodefile}{javascript}{\label{code:dafny:1}Factorial function and a pre-condition violation}{Dafny}{factorial.dfy}
	...
	\vspace{1em}\\
	\todobox{Fix highlighting here before final report submission. Write a simple lexer or something..}
	\vspace{0.5em}
\end{mycodefile}
...
Dafny has some limited string support. An example is shown in listing \ref{code:dafny:2} with a function pre-condition on the length of a string. Strings in Dafny are treated as equivalent to the type \texttt{seq<char>} (that is, a sequence of characters) so whilst it is possible to retrieve e.g. the length of such a sequence, there does not appear to be any built-in regular expression support. Whilst it \emph{is} possible to write a helper function and call it from within a pre-condition or post-condition, it then becomes necessary to manually implement the regular expression check in imperative code (as in listing \ref{code:dafny:3})

\begin{mycodefile}{javascript}{\label{code:dafny:2}Factorial function and a pre-condition violation}{Dafny}{factorial.dfy}
...
\begin{mycodefile}{javascript}{\label{code:dafny:3}Manually implementing a regular expression pre-condition}{Dafny}{regex.dfy}
...
There is some limited interoperability support with other .NET code, but the general lack of a standard library or input/output support limits Dafny's potential to an existence as an interesting research language.
...
\todobox{Snyk is mainly included here because it's bad and produces lots of false positives--e.g. Jackson databind vulns in WebSignOn that are never real.}
