...
\subsubsection{LDAP Injection}\label{ex:ldapi}
...
\begin{mycodefile}{rrtlex.py:RrtLexer -x}{Demonstration of language features}{RRT}{motivation.rrt}
    \vspace{0.5em}

    String types in our language can be refined with a regular expression constraint.
    At compile-time, the type-checker will ensure that refined types are compatible when used together (e.g. in assignment,
    as illustrated, or as a function return value).
    In this example, the type checker will not discover any problems.

    Our language supports a foreign function interface for calling existing JVM code discussed in section \ref{ch:design}.
